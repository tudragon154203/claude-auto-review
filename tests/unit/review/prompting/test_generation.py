...
class ReviewGenerationTests(unittest.TestCase):
    def test_build_prompt_assembles_expected_sections(self):
        ...
        expected = (
            ...
            " Do not emit any text before the heading — no planning notes, reasoning, progress updates,"
            " or introductory sentences. Do not wrap the output in a code fence or markdown block."
            " Do not add any text after `## Verdict`. You do not have Write or Edit tools.\n"
            "\n"
            "The review is parsed by a regex-based parser. Any text outside the structure below causes"
            " findings to be lost. Use this exact structure:\n"
            "\n"
            "```markdown\n"
