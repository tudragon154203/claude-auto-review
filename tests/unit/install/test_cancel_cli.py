...
class TestCancelCli(unittest.TestCase):
    ...
    @patch("claude_auto_review.install.cancel_cli.cancel_runtime", return_value=[FAKE_ROOT / "a", FAKE_ROOT / "b"])
    @patch("claude_auto_review.install.cancel_cli.get_client_id", return_value="test-client")
    @patch("claude_auto_review.install.cancel_cli.get_project_root", return_value=FAKE_ROOT / "project")
