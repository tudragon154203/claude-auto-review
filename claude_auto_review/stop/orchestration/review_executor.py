...
def resolve_prompted_review(
    ctx: RuntimeContext, timeout_hours: float, files_str: str, result: Any, *, emitter: ResponseEmitter
) -> StopFlowResolution:
    """Reload state after prompt execution and determine the resolution."""
    state, unreviewed = _reload_client_state(ctx)
    if not unreviewed:
        approve_no_unreviewed_after_review(ctx, emitter=emitter)
...
def execute_review_prompt(
    ctx: RuntimeContext, unreviewed: list[Any], timeout_hours: float, review_prompt_script: str, files_str: str, *, emitter: ResponseEmitter
) -> StopFlowResolution:
    ...
    try:
        ...
        return fail_review(ctx, files_str, EXIT_REVIEW_FAILED, "stop_hook_review_timeout", script=review_prompt_script, emitter=emitter)
    except (OSError, ValueError, subprocess.SubprocessError) as error:
        return fail_review(ctx, files_str, EXIT_REVIEW_FAILED, "stop_hook_review_error", error=error, emitter=emitter)
    return resolve_prompted_review(ctx, timeout_hours, files_str, result, emitter=emitter)
