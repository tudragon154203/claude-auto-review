...
def _client_state_mtime_hours(state_path, project_root=None):
    try:
        ...
        if project_root is not None:
            log_failure(
                project_root, "runtime_cleanup_failed", error, operation="stat_state_parent", target=str(state_path)
            )
        return None
...
def cleanup_stale_clients(project_root=None):
    ...
    for client_dir in clients_dir.iterdir():
        ...
        if _is_client_stale(state_path, timeout_hours, project_root=project_root) and _remove_tree(
            client_dir, project_root=project_root
        ):
